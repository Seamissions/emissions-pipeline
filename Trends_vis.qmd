...
missing_species_key <- data.frame(species_identifier = unique(missing_species$species_identifier)) %>%
  ...
  left_join(mini_key, by = c("isscaap_group_en" = "isscaap_group")) %>%
  select(species_identifier, name_en, scientific_name, major_group, isscaap_group_en, isscaap) %>%
  rename(identifier = species_identifier,
         isscaap_group = isscaap_group_en)
  
mini_key <- species_key %>%
  select(isscaap_group, isscaap) %>%
  distinct(isscaap_group, isscaap)

full_species_key <- rbind(species_key, missing_species_key)

write_csv(full_species_key, file.path("/capstone/seamissions/data/full_species_key.csv"))
