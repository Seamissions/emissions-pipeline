...
error <- diff/em_before # 0.000004253729% nice

if (error > 0.001) {
  warning("Error over 0.1% returned for emissions partitioning. Some CO2 emissions may be lost.")
}

# Total co emissions before factoring
em_before <- sum(broadcasting$emissions_co_mt, na.rm = TRUE) + sum(non_broadcasting$emissions_co_non_broadcasting_mt, na.rm = TRUE)

# Total co emissions after factoring
em_after <- sum(emissions$total_co, na.rm = TRUE)

# Difference in co emissions
diff <- em_after - em_before

# Percent error for co
error <- diff/em_before # 0.00000449349% nice

if (error > 0.001) {
  warning("Error over 0.1% returned for emissions partitioning. Some CO emissions may be lost.")
}
```
...
write_csv(emissions, here("output", "emissions.csv"))
```

Future work:

- Create for loop to check all emissions columns for error.
