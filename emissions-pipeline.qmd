...
overlapping_pixels <- st_intersects(emissions_grid_sf, fao_regions) #sparse = FALSE)
...
emissions_zones$region <- sapply(overlapping_pixels[lengths(overlapping_pixels) > 0], function(x) {
  if (length(x) > 0) {
    return(paste(fao_regions$zone[x], collapse = ","))  # Store multiple region numbers as a comma-separated string
  } else {
    return(NA)
  }
})
