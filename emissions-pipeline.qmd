...
fao_catch <- left_join(fao_capture_join, fao_waterarea_clean, by = join_by(area_code == code))

# -------- CHECKPOINT --------
# save locally
write_csv(fao_catch, here("checkpoint", "fao_cath.csv"))
...
emissions_grid_sf_merge <- emissions_grid_sf %>%
...
emissions_regions <- st_intersection(emissions_grid_sf_merge, fao_regions, sparse = FALSE)
...
fao_catch <- fao_capture_join %>%
  ...
  summarise(sum_catch = sum(catch, na.rm = TRUE)) %>%
  filter(period >= 2015) %>%
...
# flags in fao catch but not emissions
