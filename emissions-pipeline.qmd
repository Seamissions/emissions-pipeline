...
data_pathway <- "/home/emlab/projects/current-projects/ocean-ghg-fisheries/data/raw"
```
...
emissions_sf_2 <- emissions %>%
  filter(lat_bin >= -15 & lat_bin <= -75) %>%
  filter(lon_bin >= 120 & lon_bin <= -115) %>%
  st_as_sf(coords = c("lon_bin", "lat_bin"), crs = 4326) # Change CRS later
...
# force to spatial object
#emissions_grid_sf_2 <- st_as_sf(emissions_grid_2)
#emissions_grid_sf_2 <- st_sf(geometry = emissions_grid_2)
...
#fao_regions <- st_read(file.path("/home/emlab/projects/current-projects/ocean-ghg-fisheries/data/raw/world_fao_zones/world_fao_zones.shp"))

fao_regions <- st_read(file.path("/capstone/seamissions/data/fao_region_shapefile"), crs = st_crs(emissions_grid))

#st_crs(fao_regions) <- st_crs(emissions_grid_2)
