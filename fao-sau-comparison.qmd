...
# emissions_partitioned_grouped_filtered <- emissions_partitioned_grouped %>%
#   filter(year <= 2022)
...
# Join summaries
global_flag_emissions_comparison <- full_join(fao_summary, sau_summary, by = "flag") %>%
  mutate(percent_reported_co2 = round((fao_total_co2_mt/sau_total_co2_mt)*100, 2),
         tonnage_discrepancy_co2 = round(fao_total_co2_mt - sau_total_co2_mt, 2)) %>%
...
# fao_summary %>%
#   arrange(desc(fao_total_co2_mt)) %>%
#   slice(1:10) %>%
#   ggplot(aes(reorder(flag, -fao_total_co2_mt), fao_total_co2_mt)) +
#   geom_col() +
#   scale_y_continuous(labels = scales::comma) +
#   labs(x = "Flag",
#        y = "Emissions CO2 (metric tons)",
#        title = "Top 10 Global Emittors (FAO)") +
#   theme_minimal()
# 
# sau_summary %>%
#   arrange(desc(sau_total_co2_mt)) %>%
#   slice(1:10) %>%
#   ggplot(aes(reorder(flag, -sau_total_co2_mt), sau_total_co2_mt)) +
#   geom_col() +
#   scale_y_continuous(labels = scales::comma) +
#   labs(x = "Flag",
#        y = "Emissions CO2 (metric tons)",
#        title = "Top 10 Global Emittors (SAU)") +
#   theme_minimal()

global_flag_emissions_comparison %>%
...
global_flag_catch_comparison <- full_join(fao_summary, sau_summary, by = "flag") %>%
...
global_flag_catch_comparison %>%
  ...
  labs(x = "Flag",
       y = "Catch (tons)",
       fill = "Agency",
       title = "Flag Catch Disparity by Reporting Agency") +
  theme_minimal()
```
...
```{r}
# -------- Emissions --------
# Summarise FAO emissions by region
fao_summary <- full_emissions_fao_filtered %>%
  group_by(zone) %>%
  summarise(fao_total_co2_mt = sum(total_co2_mt, na.rm = TRUE))
...
# Join summaries
global_region_emissions_comparison <- full_join(fao_summary, sau_summary, by = "zone") %>%
  pivot_longer(cols = 2:3, names_to = "entity", values_to = "co2_emissions_mt") %>%
  arrange(desc(co2_emissions_mt))

global_region_emissions_comparison %>%
  slice(1:12) %>%
  ggplot(aes(reorder(zone, -co2_emissions_mt), co2_emissions_mt, fill = entity)) +
           geom_bar(stat = "identity", position = "dodge") +
  scale_y_continuous(labels = scales::comma) +
  scale_fill_manual(labels = c("FAO", "SAU"), values = c("cornflowerblue", "firebrick")) +
  labs(x = "Zone",
       y = "CO2 Emissions (metric tons)",
       fill = "Agency",
       title = "Emissions Disparity by Region") +
  theme_minimal()
...
zone_catch_comparison%>%
  ggplot(aes(reorder(zone, -catch_tons), catch_tons, fill = entity)) +
           geom_bar(stat = "identity", position = "dodge") +
  scale_y_continuous(labels = scales::comma) +
  scale_fill_manual(labels = c("FAO", "SAU"), values = c("cornflowerblue", "firebrick")) +
  labs(x = "Zone",
       y = "Catch (tons)",
       fill = "Agency",
       title = "Regional Catch Disparity by Reporting Agency") +
  theme_minimal()
...
# Join summaries
year_emissions_comparison <- full_join(fao_summary, sau_summary, by = "year") %>%
  pivot_longer(cols = 2:3, names_to = "entity", values_to = "co2_emissions_mt") %>%
  arrange(desc(co2_emissions_mt))

year_emissions_comparison %>%
  slice(1:20) %>%
  ggplot(aes(as.factor(year), co2_emissions_mt, fill = entity)) +
           geom_bar(stat = "identity", position = "dodge") +
  scale_y_continuous(labels = scales::comma) +
  scale_fill_manual(labels = c("FAO", "SAU"), values = c("cornflowerblue", "firebrick")) +
  labs(x = "",
       y = "CO2 Emissions (metric tons)",
       fill = "Agency",
       title = "Emissions Disparity by Reporting Agency Over Time") +
  theme_minimal()
...
species_key <- read_csv(file.path("/capstone/seamissions/data/master_species_key.csv"), show_col_types = FALSE) %>%
  ...
  add_row(identifier = 99999999, name_en = "non-catch associated emissions", scientific_name = "non-catch associated emissions", major_group = "non-catch associated emissions", isscaap_group = "non-catch associated emissions", isscaap = 99999999)
