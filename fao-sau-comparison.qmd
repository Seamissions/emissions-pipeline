...
# Join summaries
zone_flag_emissions_catch <- full_join(fao_summary, sau_summary, by = c("zone", "flag"))
...
fao_summary <- full_emissions_fao_species_filtered %>%
...
fao_summary %>%
 ...
 ggplot(aes(reorder(isscaap_group, fao_emissions_unit_catch), fao_emissions_unit_catch)) +
  ...
  coord_flip() +
  scale_y_continuous(labels = scales::comma) +
  labs(x = "ISSCAAP Group",
       y = "Emissions per Unit Catch",
       title = "Emissions per Unit Catch by ISSCAAP Group") +
  theme_minimal()
```
